...
// Nico: [Store]
// here we create a command to let store work.
// GIMI's store command design is not universal but i have no time optimize it, just use their design here.
// To maxmize execute speed we need to put ini parse in ParseStoreCommand, here only execute run.
class GIMIStoreCommand : public VariableAssignment {
public:
