...
static bool ParseIniPreamble(wstring *wline, wstring *ini_namespace)
{
	...
	//Nico 在这里添加了ParseIniPreamble 标记，我觉得这玩意可以不用提示把
	IniWarning("ParseIniPreamble WARNING: d3dx.ini entry outside of section: %S\n",
			wline->c_str());
	return true;
}
...
static void ParseIniStream(istream *stream, const wstring *_ini_namespace)
{
	...
	while (stream->get(ch)) {
		decryptedStream.put(ch);
	}
	istringstream decryptedInputStream(decryptedStream.str());
	

	//while (std::getline(*stream, aline)) {
	while (std::getline(decryptedInputStream, bline)) {
		...
		wline = wstring(bline.begin(), bline.end());
...
void LoadConfigFile()
{
	...
	if (GetPrivateProfileInt(L"Logging", L"calls", 0, iniFile))
	{
		if (!LogFile)
