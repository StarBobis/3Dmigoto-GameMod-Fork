...
bool RegisterIniKeyBindingDIY(LPCWSTR app, LPCWSTR iniKey,
	InputCallback down_cb, InputCallback up_cb, int auto_repeat,
	void* private_data,wstring keyName)
{
	shared_ptr<InputCallbacks> callbacks = make_shared<InputCallbacks>(down_cb, up_cb, private_data);

	RegisterKeyBinding(iniKey, keyName.c_str(), callbacks, auto_repeat, 0, 0);
	return true;
}
