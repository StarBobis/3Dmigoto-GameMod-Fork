...
//Nico:
//here we parse section
GIMIStoreCommand::GIMIStoreCommand(wstring variable_name_in, wstring resource_name_in, wstring resource_index_in){
	this->variable_name = variable_name_in;
	this->resource_name = resource_name_in;  
	this->resource_index = std::stoi(resource_index_in); //转为int格式
	this->healthVal = 0;
}
...
void GIMIStoreCommand::run(CommandListState* state) {
	...
	//TODO 这里需要解析具体是使用哪个ConstantBuffer，常用的有PS,VS,CS等等，从而获取到对应的ID3D11Buffer到buffers变量
	std::wstring shaderType = this->resource_name.substr(0,1);
	std::wstring slotStr = this->resource_name.substr(this->resource_name.length() - 1);
	...
	for (i = 0; i < D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT ; i++) {
		...
		if (i == slotNumber) {
			...
			if (dim == D3D11_RESOURCE_DIMENSION_BUFFER) {
				...
				if (FAILED(hr)) {
					//LogOverlayW(LOG_WARNING,L"[Store]Failed to CreateBuffer staging");
					continue;
				}

				state->mOrigContext1->CopyResource(staging, buffers[i]);

				// put resource into map
				D3D11_MAPPED_SUBRESOURCE map;
				hr = state->mOrigContext1->Map(staging, 0, D3D11_MAP_READ, 0, &map);
				if (FAILED(hr)) {
					LogOverlayW(LOG_WARNING, L"[Store]Failed to Map staging resource");
					continue;
				}
				...
				//输出看一下
				//wstring tips = L"[Store] ps-cb0 length: " + std::to_wstring(orig_desc.ByteWidth);
				//LogOverlayW(LOG_INFO, const_cast<wchar_t*> (tips.c_str()));

				this->healthVal = buf[this->resource_index];
		...
		DescType *desc)
{
	...
	//测试这里强行改为新的Desc呢？ 测试完成了，没有任何效果，恢复原状。
	//src_resource->GetDesc(&old_desc);
	//hr = (state->mOrigDevice1->*CreateResource)(&old_desc, NULL, &resource);
...
//{
//	D3D11_BUFFER_DESC modifiedDesc;  // 创建新的对象
//
//	// 将原始对象的值复制到新对象中
//	memcpy(&modifiedDesc, desc, sizeof(D3D11_BUFFER_DESC));
//
//	switch (override_type) {
//	case CustomResourceType::STRUCTURED_BUFFER:
...
//		modifiedDesc.StructureByteStride = override_stride;
//	else if (override_format != (DXGI_FORMAT)-1 && override_format != DXGI_FORMAT_UNKNOWN)
//		modifiedDesc.StructureByteStride = dxgi_format_size(override_format);
//
//	if (override_byte_width != -1)
//		modifiedDesc.ByteWidth = override_byte_width;
//	else if (override_array != -1)
//		modifiedDesc.ByteWidth = modifiedDesc.StructureByteStride * override_array;
//
//	if (override_bind_flags != CustomResourceBindFlags::INVALID)
//		modifiedDesc.BindFlags = (D3D11_BIND_FLAG)override_bind_flags;
//
//	if (override_misc_flags != ResourceMiscFlags::INVALID)
//		modifiedDesc.MiscFlags = (D3D11_RESOURCE_MISC_FLAG)override_misc_flags;
//
//	// 将修改后的新对象的值复制回原始对象
//	memcpy(desc, &modifiedDesc, sizeof(D3D11_BUFFER_DESC));
//}
...
static ID3D11Resource * inter_device_resource_transfer(ID3D11Device *dst_dev, ID3D11DeviceContext *dst_ctx, ID3D11Resource *src_res, wstring *name)
{
	...
	switch (dimension) {
		case D3D11_RESOURCE_DIMENSION_BUFFER:
		{
			...
			//Nico:这里疑似替换修改后的ByteWidth
			//buf_desc.ByteWidth = buf_desc.ByteWidth * 2;
...
//Nico: helper function to remove space,don't have time to find and use 3dmigoto's one.
void removeWhitespace(std::wstring& str) {
	size_t pos = 0;
	...
	// 移除结尾的空格
	pos = str.length() - 1;
	while ((pos = str.find_last_of(L' ', pos)) != std::wstring::npos) {
		str.erase(pos, 1);
		pos--;
	}
}
...
bool ParseCommandListGIMIStore(const wchar_t* section,
	...
	if (!wcscmp(key, L"store")) {
		...
		wstring variableName = tokens[0];   //$health
		removeWhitespace(variableName);
		wstring resourceName = tokens[1];	//ps-cb0
		removeWhitespace(resourceName);
		wstring resourceIndex = tokens[2];	//33   (从0开始的索引)
		removeWhitespace(resourceIndex);


		GIMIStoreCommand* command = NULL;
		CommandListVariable* var = NULL;
		wstring name = variableName.c_str();

		//如果找不到这个变量名，则报错退出
		if (!find_local_variable(name, pre_command_list->scope, &var) &&
			!parse_command_list_var_name(name, ini_namespace, &var)) {
			if (name == L"$health") {
				LogOverlayW(LOG_WARNING, L"Can't parse Store command:\n");
			}
			return false;
		}
		...
		UINT buf_size)
{
	...
	case ResourceCopyTargetType::VERTEX_BUFFER:
		...
		//TODO 在执行到这里之前，我们必须确定byte_width真的会改变那个Buffer的描述
		if (1) {
		...
		UINT *buf_dst_size)
{
	...
	if (bind_flags & D3D11_BIND_CONSTANT_BUFFER) {
		...
		*buf_dst_size = new_desc.ByteWidth;
		...
		//这里添加对VertexBuffer的特殊处理 如果是VertexBuffer的重构则使用src的ByteWidth
		//试试呗，这里确实没毛病啊，确实能执行到，但是为什么顶点数量还是原来那么多？
		//LogOverlayW(LOG_WARNING,L"D3D11_BIND_VERTEX_BUFFER recreate length");
		*buf_dst_size = new_desc.ByteWidth;
		//经过测试这里的大小确实可以改变，但是问题新创建的Buffer的ByteWidth的描述没有变，所以导致Draw的时候读取不到呀。
		
		//理论上需要执行下面的才行,但是执行下面的就会导致报错
		//dst->custom_resource->OverrideBufferDesc(&new_desc);

		//试试在其它地方，比如后面创建的地方来修改这个ByteWidth呢？
		//dst->custom_resource->OverrideBufferDesc(&new_desc);

		//这里已经测试了，修改这里的desc的确不会导致崩溃，但是后面创建Buffer的时候会导致崩溃
		//还是那个老原因，我们的3Dmigoto并没有权限来访问额外的内存空间

		//不管是hook还是wrapper的方式，都没权限。
	}
...
void ResourceCopyOperation::run(CommandListState *state)
{
	...
	if (options & ResourceCopyOptions::COPY_MASK) {
		...
		//经测试使用Copy必定会触发这里
		//LogOverlayW(LOG_WARNING,L"CopyMask Test");
		...
		if (options & ResourceCopyOptions::COPY_DESC) {
			...
			//LogOverlayW(LOG_WARNING, L"COPY_DESC Test");
