...
{
	D3D11_UNORDERED_ACCESS_VIEW_DESC modifiedDesc = *pDesc;  // 复制原始的 UAV 描述
	...
	HRESULT hr = pResource->QueryInterface(__uuidof(ID3D11Buffer), reinterpret_cast<void**>(&pBuffer));
	if (SUCCEEDED(hr))
	{
		...
		// 如果是缓冲区维度的 UAV，将顶点数量更改一下
		if (pDesc->ViewDimension == D3D11_UAV_DIMENSION_BUFFER && 
			pDesc->Format == DXGI_FORMAT_R32_TYPELESS 
			&& pDesc->Buffer.Flags == 0x1)
		{
			modifiedDesc.Buffer.NumElements = vertexNumber;

			//LogOverlayW(LOG_WARNING, L"Break UAV vertex count!");
		}
		pBuffer->Release();
	}

	//VertexLimitRaise
	//已经可以确定了，就是这个地方，这里的NumElements控制了从cs-t0,cs-t1读取的大小。
	//测试完成，完美收工。
	return mOrigDevice1->CreateUnorderedAccessView(pResource, &modifiedDesc, ppUAView);
}
...
static void override_resource_desc(D3D11_BUFFER_DESC *desc, TextureOverride *textureOverride) {
	...
	//desc->ByteWidth = textureOverride->BreakNumber;
	//std::string breakNumberStr = std::to_string(textureOverride->BreakNumber);
	//char* charPtr = new char[breakNumberStr.length() + 1];
	//std::strcpy(charPtr, breakNumberStr.c_str());
	//LogOverlay(LOG_WARNING, charPtr);
}
	...
	if (origDesc && !matches.empty()) {
		...
		*newDesc = *origDesc;
		...
		for (i = 0; i < matches.size(); i++) {
			...
			if (iniSection.find(targetSuffix) != std::string::npos) {
			//这里更换为使用find来查找是否存在，而不是作为后缀。来兼容VertexLimitRaise不在结尾的Mod
				int breakNumber = 15200000;

				try {
					//eg:[TextureOverride_VB_22ebf6e3_40_65582_VertexLimitRaise]  here 66582 is the vertex number to break.
					//std::wstring str = iniSection;

					std::vector<std::wstring> splits = splitWString(iniSection,L'_');

					//下划线之后数量小于三
					if (splits.size() > 3) {
						std::wstring numberVertexBreak = splits[splits.size() - 2];
						std::wstring numberPositionStride = splits[splits.size() - 3];

						bool isNumber = true;
						for (wchar_t c : numberVertexBreak) {
							if (!std::iswdigit(c)) {
								isNumber = false;
								break;
							}
						...
						for (wchar_t c : numberPositionStride) {
							if (!std::iswdigit(c)) {
								isNumber = false;
								break;
							}
						...
						if (isNumber) {
							...
							breakNumber = std::stoi(numberVertexBreak) * std::stoi(numberPositionStride);
				...
				//LogOverlayW(LOG_WARNING,L"VertexLimitRaise!");
				override_resource_desc(newDesc, textureOverride);
			}
		}
	}
	...
	__out_opt  ID3D11Buffer **ppBuffer)
{
	...
	if (hr == S_OK && ppBuffer && *ppBuffer)
	{
		EnterCriticalSectionPretty(&G->mResourcesLock);
			...
			handle_info->dump_hash = hash;
			//LogOverlayW(LOG_WARNING,L"Hash copied");
