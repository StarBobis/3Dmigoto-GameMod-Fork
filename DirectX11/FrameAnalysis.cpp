		...
		wchar_t *reg, char shader_type, int idx, ID3D11Resource *handle)
{
	...
	if (hash) {
		...
		if (G->track_texture_updates == 1) {
			...
			try {
				//尝试读取当前handle的hash值
				StringCchPrintfExW(pos, rem, &pos, &rem, NULL, L"=%08x", G->mResourceInfo);
			}
			catch (std::out_of_range) {
				hash = orig_hash = 0;
			}
...
HRESULT FrameAnalysisContext::FrameAnalysisFilenameResource(wchar_t *filename, size_t size, const wchar_t *type, ID3D11Resource *handle, bool force_filename_handle)
{
	...
	if (hash) {
		...
		if (G->track_texture_updates == 1) {
			...
			try {
				//尝试读取当前handle的hash值
				StringCchPrintfExW(pos, rem, &pos, &rem, NULL, L"=%08x", G->mResources.at(handle).dump_hash);
			}
			catch (std::out_of_range) {
				hash = orig_hash = 0;
			}
		...
		__in  UINT ThreadGroupCountZ)
{
	...
	//if (ThreadGroupCountY == 1 && ThreadGroupCountZ == 1) {
	//	FrameAnalysisLog("Dispatch(ThreadGroupCountX:%u, ThreadGroupCountY:%u, ThreadGroupCountZ:%u)\n",
	//		ThreadGroupCountX * 2, ThreadGroupCountY, ThreadGroupCountZ);

	//	HackerContext::Dispatch(ThreadGroupCountX * 2, ThreadGroupCountY, ThreadGroupCountZ);
	//}
	//else {
	//	FrameAnalysisLog("Dispatch(ThreadGroupCountX:%u, ThreadGroupCountY:%u, ThreadGroupCountZ:%u)\n",
